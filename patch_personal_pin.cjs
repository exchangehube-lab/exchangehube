...
`;
code = code.replace("const handleDelete = async (forAll: boolean) => {", handlePin + "\n\n  const handleDelete = async (forAll: boolean) => {");

// Add Pin to hover buttons
// In PersonalChatWindow, either participant can pin.
...
const timeStringRegex = /<span className="text-\[10px\] text-white\/40">\{timeString\}\{msg\.edited && ' \(Edited\)'\}<\/span>/;
const newTimeString = `<span className="text-[10px] text-white/40 flex items-center gap-1">{msg.pinned && <Pin className="w-2.5 h-2.5 text-purple-400" />}{timeString}{msg.edited && ' (Edited)'}</span>`;
              ...
              {messages.filter(m => m.pinned).map(msg => (
                ...
                >
                  <div className="text-[10px] text-white/40 mb-1 truncate">
                    {msg.sender_id === currentUser?.uid ? 'You' : targetUser?.username || 'user'}
                  </div>
