...
export const typingService = {
  async updateTypingStatus(userId: string, isTyping: boolean, channelId?: string, chatId?: string) {
    try {
      const id = channelId ? \`channel_\${channelId}_\${userId}\` : \`chat_\${chatId}_\${userId}\`;
      if (isTyping) {
  ...
  subscribeToChatTyping(chatId: string, callback: (payload: any) => void) {
    const q = query(collection(db, 'typing_status'), where('chat_id', '==', chatId));
    const unsub = onSnapshot(q, (snapshot) => {
      snapshot.docChanges().forEach((change) => {
        const payload = {
          eventType: change.type === 'added' ? 'INSERT' : change.type === 'modified' ? 'UPDATE' : 'DELETE',
