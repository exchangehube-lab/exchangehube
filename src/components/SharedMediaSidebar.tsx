                ...
                >
                  ...
                  <div className="flex flex-col min-w-0 flex-1">
                    ...
                    <span className="text-[10px] text-white/40 flex items-center gap-1">
                      {formatSize(msg.file_size)} • {getSenderName(msg.sender_uid)} • {format(new Date(msg.created_at), 'MMM d, yyyy')}
                    </span>
                  </div>
              ...
              {voiceMessages.map(msg => (
                <div key={msg.id} className="flex flex-col gap-2 p-3 bg-white/5 rounded-xl">
                  <div className="flex items-center gap-2">
                    ...
                    <span className="text-xs text-white/70 font-medium truncate flex-1">{getSenderName(msg.sender_uid)}</span>
              ...
              {linkMessages.map((msg, i) => (
                ...
                >
                  ...
                  <div className="flex flex-col min-w-0 flex-1">
                    ...
                    <span className="text-[10px] text-white/40 flex items-center gap-1">
                      {getSenderName(msg.sender_uid)} • {format(new Date(msg.created_at), 'MMM d, yyyy')}
                    </span>
                  </div>
                </a>
              ))}
            ...
            <div className="absolute bottom-6 left-0 right-0 flex justify-center">
              <div className="bg-[#1A1D2D]/80 backdrop-blur-md border border-white/10 px-4 py-2 rounded-xl flex items-center gap-4">
                <div className="flex flex-col">
                  <span className="text-sm font-medium text-white">{getSenderName(fullScreenMedia.sender_uid)}</span>
